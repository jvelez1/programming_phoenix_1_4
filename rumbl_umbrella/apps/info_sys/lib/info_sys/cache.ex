defmodule InfoSys.Cache do
  ...
  def init(opts) do
    ...
    {:ok, schedule_clear(state)} # Cache expiration
  end
  ...
  def put(name \\ __MODULE__, key, value) do
    true = :ets.insert(tab_name(name), {key, value}) # cache saving data
    :ok
  end

  def fetch(name \\ __MODULE__, key) do
    {:ok, :ets.lookup_element(tab_name(name), key, 2)} # cache fetching data
  rescue
    ArgumentError -> :error
  end
